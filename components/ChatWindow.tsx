...
export function ChatWindow(props: {
  ...
  const {
    ...
    onResponse(response) {
      ...
      const unsortedSources = sourcesHeader
        ? JSON.parse(Buffer.from(sourcesHeader, "base64").toString("utf8"))
        : [];

      const sources = unsortedSources.sort((a: any, b: any) =>
        a.metadata.documentType === "SERVICE_AGREEMENT"
          ? -1
          : b.metadata.documentType === "SERVICE_AGREEMENT"
          ? 1
          : a.metadata.documentType === "ADMINISTRATIVE_CIRCULAR"
          ? -1
          : b.metadata.documentType === "ADMINISTRATIVE_CIRCULAR"
          ? 1
          : 0,
      );
      const messageIndexHeader = response.headers.get("x-message-index");
      console.log("FOO sources", sources);
  ...
  return (
    <div
      className={`flex flex-col items-center py-4 md:py-8 md:px-16 px-4 grow overflow-hidden bg-[#F8B200] ${
        messages.length > 0 ? "border" : ""
      }`}
    >
      <h2 className={`${messages.length > 0 ? "" : "hidden"} text-2xl mb-4`}>
        {titleText}
      </h2>
      ...
      <form onSubmit={sendMessage} className="flex w-full flex-col">
        ...
        <div className="flex w-full mt-4">
          ...
          <button
            type="submit"
            className="shrink-0 px-8 py-4 bg-slate-900 w-28 uppercase font-bold"
          >
            <div
              role="status"
              className={`${
